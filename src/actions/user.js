    ...
    return axios
      ...
      .then(response => {
        ...
        if (response.status === 200) {
          if (Number(response.data.expires_in) > 0) {
...
export const save = (userData, data) => {
  userData.coins = Number(userData.coins) + Number(data.amount);
  userData.saving = Number(userData.saving) + Number(data.amount);

  userData.goals.forEach(function(goal) {
    if (goal.isActive) {
      goal.currentAmount = Number(goal.currentAmount) + Number(data.amount);
      ...
      for (let i = 0; i < goal.weeks.length; ++i) {
        ...
        let cumulativeSum = 0;
        for (let j = 0; j <= i; ++j) {
          cumulativeSum = Number(cumulativeSum) + Number(goal.weeks[j].target);
        }
        ...
        if (goal.weeks[i].achieved === undefined) {
          if (i - 1 > -1) {
            // if current week
            if (
              addDays(goal.startDate, (i - 1) * 7).getTime() <
                today.getTime() &&
              today.getTime() <= addDays(goal.startDate, (i + 1) * 7).getTime()
            ) {
              if (goal.currentAmount >= cumulativeSum) {
                goal.weeks[i].achieved = true;
                // console.log(cumulativeSum);
                userData.coins =
                  Number(userData.coins) + Number(i * goal.weeks[i].target);
              }
            } else if (
              today.getTime() <= addDays(goal.startDate, (i + 1) * 7).getTime()
            ) {
...
export const addGoal = (userData, data) => {
  const activeGoal = userData.goals.find(goal => goal.isActive);
